...
public class zookpeer {
	public static void main(String[] args) throws Exception{
		//new ZkServer("127.0.0.1:9998","127.0.0.1:2181","/servers",30000);
		Watcher watcher2 = new Watcher(){
			public void process(WatchedEvent event) {
				LOGGER.debug(event.getPath()+" :"+event.getType().name());
			}
		};
		ZooKeeper zk2 = new ZooKeeper("127.0.0.1:2181", 30000, watcher2);
		List<String> children = zk2.getChildren("/servers", false);
		for (String string : children) {
			System.out.println(string);
		} 
		 byte[] data = zk2.getData("/servers", false, null);
	...
	public zookpeer(String server, String zkServeres,String serverPath){
		this(server, zkServeres, serverPath, 20000);
	}
	
	public zookpeer(String monitorServer, String zkServeres, String serverPath, int timeout){
